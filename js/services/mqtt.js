(function() {
  angular.module('Mqtt.Services', [])
    .factory('mqtt', function() {
      ...
      mqttObj.connect = function(host, port, user, pass, useSSL,
        clientId) {
        ...
        if (connections[key] === undefined && currentAttempts[key] ===
          undefined) {
          // try and help the race condition
          currentAttempts[key] = {};
          ...
          client.onConnectionLost = function(responseObject) {
            ...
            retry = setInterval(function() {
              console.log('lost connection to MQTT broker ' + host +
                ', retrying now');
              mqttObj.connect(host, port, user, pass, useSSL,
                clientId);
            }, 5000);
          };
          ...
          client.connect({
            onSuccess: function() {
              // got connected, kill auto-retry for now
              clearInterval(retry);
              console.log('onSuccess for connect to ' + host);
              connections[key] = client;
              currentAttempts[key] = undefined;
              var ts = toSubscribe[key];
              if (ts !== undefined) {
                for (var x = 0; x < ts.length; x++) {
                  mqttObj.subscribe(ts[x].topic, ts[x].callback,
                    host,
                    port, user, pass, useSSL, clientId);
                }
                toSubscribe[key] = [];
              }
              var tse = toSend[key];
              if (tse != undefined) {
                for (var x = 0; x < tse.length; x++) {
                  mqttObj.sendMessage(tse[x].msg, tse[x].topic, host,
                    port, user, pass, useSSL);
                }
                toSend[key] = [];
              }
            },
            onFailure: function() {
              ...
              clearInterval(retry);
              retry = setInterval(function() {
                console.log('failed connection to MQTT broker ' +
                  host +
                  ', retrying now');
                mqttObj.connect(host, port, user, pass, useSSL,
                  clientId);
      ...
      mqttObj.subscribe = function(topic, callback, host, port, user,
        pass,
        useSSL, clientId) {
        if (useSSL === undefined) useSSL = false;
        var key = getHashKey(host, port, user, pass, useSSL);
      ...
      mqttObj.sendMessage = function(msg, topic, host, port, user,
        pass,
        useSSL) {
        if (useSSL === undefined) useSSL = false;
        var key = getHashKey(host, port, user, pass, useSSL);
